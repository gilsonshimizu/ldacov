...
`gibbs.LDA.cov` requires an estimate of the matrix $\Phi$. We obtain this quantity by refitting the LDA model with no covariates using 4 clusters:
...
We can now fit the LDA model with covariates:
